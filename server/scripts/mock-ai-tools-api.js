...
app.post(
  ...
  async (req, res) => {
    ...
    if (mode === "error") {
      return res
        .status(500)
        .json({ ok: false, error: "mock upstream failure" });
    }
...
app.listen(port, () => {
  ...
  console.log(
    `Searchable PDF: POST http://localhost:${port}${ENDPOINTS[TOOLS.SEARCHABLE_PDF]}`
  );
  console.log(`X-ray: POST http://localhost:${port}${ENDPOINTS[TOOLS.XRAY]}`);
  console.log(
    `LLM Recorder: POST http://localhost:${port}/v1/chat/completions`
  );
