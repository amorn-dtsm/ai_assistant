    ...
    <div className="flex gap-x-[5px] w-full items-center h-[32px]">
      <div className="relative h-full w-full flex">
        <input
          ...
          className="border-none w-full h-full rounded-lg bg-theme-sidebar-item-default pl-9 focus:pl-4 pr-1 placeholder:text-white/50 light:placeholder:text-slate-500 green:placeholder:text-[#B1D3C2] placeholder:font-semibold outline-none text-theme-text-primary search-input peer text-sm"
        />
        <MagnifyingGlass
          size={14}
...
function SearchResultWrapper({ children }) {
  return (
    <div className="absolute right-0 top-[6.2%] w-full flex flex-col gap-y-[24px] h-auto bg-theme-modal-border light:bg-theme-bg-primary light:border-2 light:border-theme-modal-border green:bg-[#036735] green:border-2 green:border-[#025029] rounded-lg p-[16px] z-10 max-h-[calc(100%-24px)] overflow-y-scroll no-scroll">
      {children}
    </div>
  );
}
...
function SearchResultItem({ to, name, hint }) {
  return (
    <Link
      ...
      className="hover:bg-[#FFF]/10 light:hover:bg-[#000]/10 green:hover:bg-white/10 transition-all duration-300 rounded-sm px-[8px] py-[2px]"
    >
  ...
  return (
    <>
      <button
        ...
        className="border-none flex items-center justify-center bg-white  rounded-lg p-[8px] hover:bg-white/80 light:hover:bg-slate-300 green:hover:bg-[#B1D3C2] transition-all duration-300"
      >
        <Plus
          size={16}
          weight="bold"
          className="text-black light:text-slate-500 green:text-[#025029]"
        />
      </button>
      <Tooltip
